...
export function rollRivalTraderOffer(state: EconomyState): { goodId: GoodId; qty: number; pricePerUnit: number } {
  const good = GOODS[Math.floor(Math.random() * GOODS.length)];
  const qty = Math.max(5, Math.round((good.baseSupply * (0.08 + Math.random() * 0.08)) / 5) * 5);
  const premium = RIVAL_OFFER_PREMIUM_MIN + Math.random() * (RIVAL_OFFER_PREMIUM_MAX - RIVAL_OFFER_PREMIUM_MIN);
