...
| `npm run verify` (Android only)                     | Typecheck, lint, format, i18n, native module versions, 190 tests — fails in a minute instead of at the end of a 20-minute Gradle run. Run once, on the Android workflow, rather than twice.                                          |
